...
describe('Auth JWT', () => {
  ...
  describe('Get Token', () => {
    ...
    it('should return token from session storage if local storage is empty', () => {
      sessionStorage.setItem('m-2-gdil-authenticationToken', JSON.stringify('sessionStorageToken'));
      const result = service.getToken();
      expect(result).toEqual('sessionStorageToken');
    });

    it('should return token from localstorage storage', () => {
      localStorage.setItem('m-2-gdil-authenticationToken', JSON.stringify('localStorageToken'));
      const result = service.getToken();
